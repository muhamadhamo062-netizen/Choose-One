...
type Body = { transactionId?: string };
...
export async function POST(request: Request) {
  ...
  const subRes = await safeDbResult(() =>
    prisma.subscription.findFirst({
      where: { paddleTransactionId: transactionId, plan: "lifetime", status: "active" },
      include: { user: true }
    })
  );
  if (!subRes.ok) {
  ...
  if (!sub || !sub.user) {
    return NextResponse.json(
      { ok: false, state: "NOT_READY", code: "transaction_not_confirmed" },
      { status: 404 }
    );
  }
