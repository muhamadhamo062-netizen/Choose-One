...
/**
 * Lifetime plan after verified Lemon Squeezy payment (webhook) or dev-only unverified path.
 * `billingOrderId` is stored on Subscription (DB column legacy name: paddle_transaction_id).
 */
export async function fulfillLifetimeEntitlement(input: {
  ...
  const orderId = input.billingOrderId?.trim() || null;
  ...
  if (linkScanId) {
    ...
    if (s) {
      if (s.userId) {
        ...
        if (owner && normalizeAuthEmail(owner.email) !== email) {
          throw new Error("scan_attached_to_other_user");
        }
      } else {
        const onScan = s.email?.trim() ? normalizeAuthEmail(s.email) : null;
  ...
  if (orderId) {
    const dupeRes = await safeDbResult(() =>
      prisma.subscription.findFirst({ where: { paddleTransactionId: orderId } })
    );
    if (!dupeRes.ok) {
      throw new Error("database_unavailable");
    }
    ...
    prisma.$transaction(async (tx) => {
        ...
        if (sub) {
          await tx.subscription.update({
            ...
            data: {
              plan: "lifetime",
              status: "active",
              ...(orderId ? { paddleTransactionId: orderId } : {})
            }
          });
        } else {
          await tx.subscription.create({
      ...
      if (linkScanId && scanToLink) {
        ...
        if (!already) {
          ...
          if (unclaimed) {
            const onScan = unclaimed.email?.trim() ? normalizeAuthEmail(unclaimed.email) : null;
            if (onScan && onScan !== email) {
              throw new Error("email_mismatch_with_scan");
            }
            await tx.scan.update({
  ...
  void emitServerEvent({
    ...
    payload: {
      email: userRow.email,
      source: orderId ? "lemon_squeezy_webhook" : "unverified_client",
      scanId: linkScanId || ""
    }
  });
